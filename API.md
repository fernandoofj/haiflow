...
| `model` | string | No | Model to run, passed straight to `claude --model` (alias like `sonnet`/`opus`, or a full model id). **Omitted means no `--model` at all** — the session runs the account default, and the state records `model: null` so "no choice was made" stays visible instead of implicit. A present-but-invalid value is a `400`, never a silent fallback |

Returns `{ "started": true, "session": "worker", "tmux": "worker", "cwd": "...", "ready": true, "model": "sonnet" }`. `model` is what the session actually started with, and `null` means it runs the account default.

`--model` only applies at startup — `claude` is a long-lived process. So starting a session that is **already running under a different model** returns `409` with `modelMismatch: { running, requested }` instead of reusing it: reusing would run a model nobody chose, which is the exact defect this endpoint's `model` field exists to prevent. Stop the session first, then start it with the new model. `ready` is `false` when the session linked but its TUI readiness could not be confirmed in time (still usable). When the server has a forced `HAIFLOW_CWD` that differs from the requested `cwd`, the response also includes `"cwdOverridden": true` and `cwd` reflects the forced value. When `cwd` is omitted and no `HAIFLOW_CWD` is pinned, the session starts in `/tmp` and the response includes `"cwdDefaulted": true`. With `HAIFLOW_ALLOW_REQUEST_CWD=false`, sending a `cwd` is rejected with `400`.
